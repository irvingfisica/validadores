...
with cols_ui[1]:
    encoding = st.text_input(
        f"Selecciona el encoding del archivo:",
        value="UTF8",
        help="Si el archivo tiene caracteres extraños, prueba con cp1252, cp850 o Latin1. El archivo de salida siempre será UTF8",
    )
        ...
        for idx, col in enumerate(df.columns):
            ...
            with cols_ui[0]:
                nuevo_nombre = st.text_input(
                    f"Nombre para '{col}'", value=nombre_sugerido
                )
            with cols_ui[1]:
                tipo = st.selectbox(
                    f"Tipo de datos: ",
            ...
            for col, conf in configuracion.items():
                ...
                try:
                    if conf["tipo"] == "texto":
                        ...
                        df_transformado[col] = transformar_a_numerica_coordenada(
                            df[col]
                        )
